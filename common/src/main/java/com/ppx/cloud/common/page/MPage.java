...
public class MPage {
	...
	// 不能使用set开头，防止传参数攻击
	public MPage addDefaultOrderName(String orderName) {
	...
	// 不能使用set开头，防止传参数攻
	public MPage addDefaultOrderType(String orderType) {
		this.orderType = StringUtils.isEmpty(this.orderType) ? orderType : this.orderType;
		return this;
	}
	
	// 不能使用set开头，防止传参数攻
	public MPage addPermitOrderName(String... permitOrderName) {
		permitOrderNameSet.addAll(Arrays.asList(permitOrderName));
		return this;
	}
	
	// 不能使用set开头，防止传参数攻
    public MPage addUnique(String unique) {
        this.unique = unique;
        return this;
	}
